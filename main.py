...
def main():
    ...
    # Esta é a linha que causa o aviso, pois é uma chamada assíncrona.
    # Vamos corrigi-la.
    
    # Inicia a execução do bot e o registro dos comandos
    # usando um método síncrono para evitar o RuntimeWarning.
    try:
        app.bot.set_my_commands(commands).result()
    except Exception as e:
        logger.error(f"Erro ao registrar os comandos: {e}")
        # Continua mesmo com erro, já que os CommandHandlers ainda funcionam.
